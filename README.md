...
If the `inputDir` + `outputPath` directory does not already exist, the directories after in that path will be created and the generated image with filename `fileName` included in the last directory of `outputPath`. If the given filepath already and image exists, the shortcode will throw an error because `overwrite=false` by default.
...
Make sure to use `addPassthroughCopy()` to include the directory of social share images you specify in `outputPath`. This will ensure that the directory of generated images are included in the sites output (ie `_site`) when the site is built and deployed with Netlify. 
